...
def Anim_2D_comete(V0, time_max, nb_values, Nb_corps, Mass_corps):
    ...
    colors_p = ['#FCE570', '#8c8c94', '#ebe0c6', '#6b93d6', '#c67b5c', '#d8ca9d',
                '#c4b08b', '#d1e7e7', '#5b5ddf', 'blue']  # liste couleurs de nos planètes

    solution = resolution(V0, time_max, nb_values, Nb_corps, Mass_corps)

    long_train = 1000  # taille de la trainée
    ...
    def anim(k):
        ...
        j = Nb_corps-1
        lignes[j].set_data([solution.y[2*j, k:0:-1]],
                           [solution.y[2*j+1, k:0:-1]])
        plt.title("Système solaire à t = " +
                  str(round(solution.t[k], 1)) + " ans", x=-15, y=1.35)

    lignes = [axes.plot([], [], "o-", markersize=10, markevery=10000, color=colors_p[0], lw=1)[0]]+[axes.plot([], [], "o-", markersize=4, markevery=1000000, color=colors_p[i], lw=1,)[0]
                                                                                                    for i in range(1, Nb_corps-1)]+[axes.plot([], [], "o--", markersize=4, markevery=1000000, color=colors_p[Nb_corps-1], lw=1)[0]]  # couleur de chaque planète
    saut = 10000  # sauter des frames pour accélérer l'animation
    ...
    plt.legend(lignes, ['Soleil', 'Mercure', 'Venus', 'Terre',
               'Mars', 'Jupiter', 'Saturne', 'Uranus', 'Neptune', 'comète'], bbox_to_anchor=(-35, 0, 0.5, 1))  # affichage de la légende
    '''
    ############################# Enregistrement GIF####################################
    writergif = animation.PillowWriter(fps=30)
    anim2D.save("./comete.gif", writer=writergif)
    ####################################################################################
    '''
...
msol = np.array([1.9891e30, 3.302e23, 4.8685e24, 5.9736e24,
                 6.4185e23, 1.8986e27, 5.6846e26, 8.6810e25, 1.0243e26])
c = 1E14
m = np.append(msol, c)
N = len(m)
pos0sol = np.array([0, 0, 3.52236e10, -4.5987e10, 0, 1.08208930e11, -1.495978875e11, 0, 0, -2.27936637e11,
                    7.78412027e11, 0, 0, 1.421179772e12, -2.876679082e12, 0, 0, -4.503443661e12])

pos0sol = pos0sol*(1/149597870700)
# changment d'unité
pc = np.array([0.5, 0.5])
pos0 = np.append(pos0sol, pc)
vit0sol = np.array([0, 0, 3.08175e4, 2.686828e4, -3.502e4, 0, 0, -29291, 24077,
                    0, 0, 13057.2, -9644.6, 0, 0, -6.81e3, 5431.7, 0])
vit0sol = vit0sol*(365.25*24*3600)*(1/149597870700)  # changment d'unité
theta = np.arctan(pc[0]/pc[1])
norme_vit = 10.43
vit_com = np.array([-np.cos(theta), np.sin(theta)])*norme_vit
vit0 = np.append(vit0sol, vit_com)
V0 = np.append(pos0, vit0)  # Vecteur à l'instant 0

#####################################################
if __name__ == "__main__":
    Anim_2D_comete(V0, tf, nb_valeurs, N, m)
